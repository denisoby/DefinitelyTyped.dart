...
class DartFileInfo {
    ...
    isRootFile:boolean;
    isLibrary:boolean;
    namespace: {classes: {}, filePath: string};
    knownImportedTypes: string[];
    dependencies: [];

    constructor(fileName, params?) {
        var {isRootFile, isLibrary, libraryName} = params;
    ...
    addImportForType(typeText: string, libName: string){
...
/*
                throw `Class declaration not found for '${className}' in ` +
                (libName && `'${libName}'` || " global namespace");
*/
            }
        }
        else{
...
export function main() {
    ...
    var nodeAnnotator = {
        ModuleDeclaration: function(params){
            var {node, parentNode, parentNodeName} = params;
    ...
    const source = String(fs.readFileSync(sourceFilePath));
    const sf = ts.createSourceFile(sourceFilePath, source, ts.ScriptTarget.Latest);
...
*/
    ...
    function processNodes(parentNode: ts.Node, parentNodeName: string, resultCollector?) : string {
        var result = '';
        if (!resultCollector){
        ...
        ts.forEachChild(parentNode, node => {
            ...
            if (annotatorFn) {
                var _result = annotatorFn({node, parentNode: parentNode, parentNodeName: parentNodeName});
                ...
                if (typeText.length == 1){
                    ...
                    if (foundInSystemTypes){
                        ...
                        if ((libName || classNotFoundInCurrentLib) && notFoundInImports
                            && notCurrentLib) {
                            currentDartFile.dependencies.push({typeText, libName});
                        }
                    }
                }
                break;
